...
const getImmutableObservable = (relation: Relation<any>): Observable<any> =>
  relation._model.collections
    .get(relation._relationTableName)
    .findAndObserve(relation?.id as string)

const getObservable = <T extends Model = Model>(relation: Relation<T>): Observable<T> =>
...
export const createObservable = <T extends Model = Model>(relation: Relation<T>): Observable<T> =>
