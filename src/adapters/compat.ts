...
export default class DatabaseAdapterCompat {
  ...
  batchImport(tables: TableName<any>[], srcDB: any): Promise<void> {
    return toPromise(callback => this.underlyingAdapter.batchImport(tables, srcDB, callback))
  }
  ...
  disableNativeCDC(): Promise<void> {
    return toPromise(callback => this.underlyingAdapter.disableNativeCDC(callback))
  }
